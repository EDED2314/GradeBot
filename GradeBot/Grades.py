...
class get_the_grade_from_website:
    ...
    async def get(email, password):
        ...
        view = None
        people = soup1.find("select", name="fldStudent").find_all("option")
        if len(people) > 1:
            view = View()
        ...
        return (info, grades, courses, view)
...
class Dropdown(disnake.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Choose what account you want",
            min_values=1,
            max_values=1,
            options=options,
        )
...
class GetGrade(commands.Cog):
    ...
    @commands.slash_command(name="mygrade")
    async def get_grade(
            self,
        ...
        """
        Check your grades and other stats with ease; will not respond if someone else is using it

        Parameters
        ----------
        email: your entire email.
        password: your password to login into genesis.
        """
        ...
        async with channel.typing():
            ...
            try:
                ...
                return
            if grades[3] is not None:
                await inter.channel.send(message="Please select a user!", view=grades[3])
            new_data = {}
            info = grades[0]
            ...
            embed = disnake.Embed(title=f"{inter.author.name}'s School Status", description=description, color=disnake.Color.blurple())
